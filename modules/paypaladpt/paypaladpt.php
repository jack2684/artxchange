...
class PayPalAdpt extends PaymentModule
{
	...
	{
		$this->name = 'paypaladpt';
		$this->version = '1.0';
		$this->author = 'Junjie Guan';
		$this->className = 'PayPalAdpt';
	...
	private function _installDb()
	{
		...
		CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'paypal_adpt_transaction` (
			`id_paypal_adpt_transaction` int(11) NOT NULL AUTO_INCREMENT,
		...
		PRIMARY KEY (`id_paypal_adpt_transaction`), KEY `idx_transaction` (`type`,`id_order`))
	...
	public function uninstall()
	{
		...
		/* $result &= Db::getInstance()->Execute('DROP TABLE `'._DB_PREFIX_.'paypal_adpt_transaction`'); */
	...
	public function getContent()
	{
		...
		if (Configuration::get('PAYPAL_USA_PAYMENT_ADVANCED') && Validate::isLoadedObject($this->_shop_country) && $this->_shop_country->iso_code != 'US')
			$this->context->smarty->assign('paypal_adpt_advanced_only_us', true);
		...
		$this->context->smarty->assign(array(
			'paypal_adpt_tracking' => 'http://www.prestashop.com/modules/paypalusa.png?url_site='.Tools::safeOutput($_SERVER['SERVER_NAME']).'&id_lang='.(int)$this->context->cookie->id_lang,
			'paypal_adpt_form_link' => './index.php?tab=AdminModules&configure=paypalusa&token='.Tools::getAdminTokenLite('AdminModules').'&tab_module='.$this->tab.'&module_name=paypalusa',
			'paypal_adpt_ssl' => Configuration::get('PS_SSL_ENABLED'),
			'paypal_adpt_validation' => (empty($this->_validation) ? false : $this->_validation),
			'paypal_adpt_error' => (empty($this->_error) ? false : $this->_error),
			'paypal_adpt_warning' => (empty($this->_warning) ? false : $this->_warning),
			'paypal_adpt_configuration' => Configuration::getMultiple(array('PAYPAL_USA_SANDBOX', 'PAYPAL_USA_PAYMENT_STANDARD', 'PAYPAL_USA_PAYMENT_ADVANCED',
			...
			'paypal_adpt_merchant_country_is_usa' => (Validate::isLoadedObject($this->_shop_country) && $this->_shop_country->iso_code == 'US'),
			'paypal_adpt_merchant_country_is_mx' => (Validate::isLoadedObject($this->_shop_country) && $this->_shop_country->iso_code == 'MX'),
			'paypal_adpt_ps_14' => (version_compare(_PS_VERSION_, '1.5', '<') ? 1 : 0),
			'paypal_adpt_b1width' => (version_compare(_PS_VERSION_, '1.5', '>') ? '350' : '300'),
			'paypal_adpt_js_files' => stripcslashes('"'._PS_JS_DIR_.'jquery/jquery-ui-1.8.10.custom.min.js","'.$this->_path.'js/colorpicker.js","'.$this->_path.'js/jquery.lightbox_me.js","'.$this->_path.'js/paypalusa.js'.'"')
		));
	...
	{
		if (!isset($_POST['paypal_adpt_products']) && !isset($_POST['paypal_adpt_express_checkout']))
			$this->_error[] = $this->l('You must choose at least one PayPal product to enable.');
		else
		{
			foreach (array(1 => 'PAYPAL_USA_PAYMENT_STANDARD', 2 => 'PAYPAL_USA_PAYMENT_ADVANCED', 3 => 'PAYPAL_USA_PAYFLOW_LINK') as $paypal_adpt_product_id => $paypal_adpt_product)
				Configuration::updateValue($paypal_adpt_product, (isset($_POST['paypal_adpt_products']) && $_POST['paypal_adpt_products'] == $paypal_adpt_product_id) ? 1 : null);

			Configuration::updateValue('PAYPAL_USA_EXPRESS_CHECKOUT', isset($_POST['paypal_adpt_express_checkout']));
	...
	{
		if (!isset($_POST['paypal_adpt_account']) || !$_POST['paypal_adpt_account'])
			$this->_error[] = $this->l('Your Paypal Business Account is required.');
		
		Configuration::updateValue('PAYPAL_USA_ACCOUNT', pSQL(Tools::getValue('paypal_adpt_account')));
		if (Configuration::get('PAYPAL_USA_EXPRESS_CHECKOUT'))
		{	
			if (!isset($_POST['paypal_adpt_api_username']) || !$_POST['paypal_adpt_api_username'])
				$this->_error[] = $this->l('Your Paypal API Username is required.');
			if (!isset($_POST['paypal_adpt_api_password']) || !$_POST['paypal_adpt_api_password'])
				$this->_error[] = $this->l('Your Paypal API Password is required.');
			if (!isset($_POST['paypal_adpt_api_signature']) || !$_POST['paypal_adpt_api_signature'])
				$this->_error[] = $this->l('Your Paypal API Signature is required.');
		}
			Configuration::updateValue('PAYPAL_USA_API_USERNAME', pSQL(Tools::getValue('paypal_adpt_api_username')));
			Configuration::updateValue('PAYPAL_USA_API_PASSWORD', pSQL(Tools::getValue('paypal_adpt_api_password')));
			Configuration::updateValue('PAYPAL_USA_API_SIGNATURE', pSQL(Tools::getValue('paypal_adpt_api_signature')));
			Configuration::updateValue('PAYPAL_USA_SANDBOX', (bool)Tools::getValue('paypal_adpt_sandbox'));

		/* PayPal Express Checkout options */
		if (Configuration::get('PAYPAL_USA_EXPRESS_CHECKOUT') && !isset($_POST['paypal_adpt_checkbox_shopping_cart']) && !isset($_POST['paypal_adpt_checkbox_product']))
			$this->_error[] = $this->l('As PayPal Express Checkout is enabled, please select where it should be displayed.');
		else
		{
			Configuration::updateValue('PAYPAL_USA_EXP_CHK_PRODUCT', isset($_POST['paypal_adpt_checkbox_product']));
			Configuration::updateValue('PAYPAL_USA_EXP_CHK_SHOPPING_CART', isset($_POST['paypal_adpt_checkbox_shopping_cart']));
			Configuration::updateValue('PAYPAL_USA_EXP_CHK_BORDER_COLOR', pSQL(Tools::getValue('paypal_adpt_checkbox_border_color')));
		}
	...
	{
		if (!isset($_POST['paypal_adpt_manager_user']) || !$_POST['paypal_adpt_manager_user'])
			$this->_error[] = $this->l('Your Paypal Manager User is required.');
		if (!isset($_POST['paypal_adpt_manager_login']) || !$_POST['paypal_adpt_manager_login'])
			$this->_error[] = $this->l('Your Paypal Manager Login is required.');
		if (!isset($_POST['paypal_adpt_manager_password']) || !$_POST['paypal_adpt_manager_password'])
			$this->_error[] = $this->l('Your Paypal Manager Password is required.');
		if (!isset($_POST['paypal_adpt_manager_partner']) || !$_POST['paypal_adpt_manager_partner'])
			$this->_error[] = $this->l('Your Paypal Manager Partner is required (Default value is "PayPal").');

		Configuration::updateValue('PAYPAL_USA_SANDBOX_ADVANCED', (bool)Tools::getValue('paypal_adpt_sandbox_advanced'));
		Configuration::updateValue('PAYPAL_USA_MANAGER_USER', Tools::getValue('paypal_adpt_manager_user'));
		Configuration::updateValue('PAYPAL_USA_MANAGER_LOGIN', Tools::getValue('paypal_adpt_manager_login'));
		Configuration::updateValue('PAYPAL_USA_MANAGER_PASSWORD', Tools::getValue('paypal_adpt_manager_password'));
		Configuration::updateValue('PAYPAL_USA_MANAGER_PARTNER', Tools::getValue('paypal_adpt_manager_partner'));
	...
	{
		$html = '';
		$paypal_adpt_express_checkout_no_token = (!isset($this->context->cookie->paypal_express_checkout_token) || empty($this->context->cookie->paypal_express_checkout_token));
		
		/* 2013-11-8 modify to adapt 1.4, PayPal Express Checkout */
		if (Configuration::get('PAYPAL_USA_EXPRESS_CHECKOUT'))
		{
			/* Display the PayPal Express Checkout button to confirm the payment */
			$this->context->smarty->assign(array('paypal_adpt_express_checkout_hook_payment' => true,
				'paypal_adpt_merchant_country_is_mx' => (Validate::isLoadedObject($this->_shop_country) && $this->_shop_country->iso_code == 'MX'),
				'paypal_adpt_express_checkout_no_token' => $paypal_adpt_express_checkout_no_token,
				'paypal_adpt_action_payment' => $this->context->link->getModuleLink('paypalusa', 'expresscheckout', array('pp_exp_payment' => 1)), 
				'paypal_adpt_action' => $this->context->link->getModuleLink('paypalusa', 'expresscheckout', array('pp_exp_initial' => 1))));
		...
		else
		{
			...
			$paypal_adpt_express_checkout_no_token = true;
		}	

		/* PayPal Payments Standard */

		if (Configuration::get('PAYPAL_USA_PAYMENT_STANDARD') && ($this->_shop_country instanceof Country && $this->_shop_country->iso_code != 'MX') && $paypal_adpt_express_checkout_no_token)
		{
			...
			$this->context->smarty->assign(array(
				'paypal_adpt_action' => 'https://www'.(Configuration::get('PAYPAL_USA_SANDBOX') ? '.sandbox' : '').'.paypal.com/cgi-bin/webscr',
				'paypal_adpt_customer' => $this->context->customer,
				'paypal_adpt_business_account' => Configuration::get('PAYPAL_USA_ACCOUNT'),
				'paypal_adpt_billing_address' => $billing_address,
				'paypal_adpt_total_tax' => (float)$this->context->cart->getOrderTotal(true) - (float)$this->context->cart->getOrderTotal(false),
				'paypal_adpt_cancel_url' => $this->context->link->getPageLink('order.php',''),
				'paypal_adpt_notify_url' => $this->context->link->getModuleLink('paypalusa', 'validation', array('pps' => 1), false), 
				'paypal_adpt_return_url' => /*26/12/2013 fix for Backward compatibilies on confirmation page*/
		...
		if ((Configuration::get('PAYPAL_USA_PAYFLOW_LINK') || Configuration::get('PAYPAL_USA_PAYMENT_ADVANCED')) && (Validate::isLoadedObject($this->_shop_country) && $this->_shop_country->iso_code != 'MX') && $paypal_adpt_express_checkout_no_token)
		{
			...
			if ((isset($result['RESULT']) && $result['RESULT']== 0) && !empty($result['SECURETOKEN']) && $result['SECURETOKENID'] == $token && Tools::strtoupper($result['RESPMSG']) == 'APPROVED')
			{
				...
				$this->context->smarty->assign('paypal_adpt_advanced_iframe_url', 'https://'.(Configuration::get('PAYPAL_USA_SANDBOX_ADVANCED') ? 'pilot-' : '').'payflowlink.paypal.com/payflowlink.do?SECURETOKEN='.$result['SECURETOKEN'].'&SECURETOKENID='.$result['SECURETOKENID'].(Configuration::get('PAYPAL_USA_SANDBOX_ADVANCED') ? '&MODE=TEST' : ''));
	...
	public function hookBackOfficeHeader()
	{
		...
		if (Tools::isSubmit('process_refund') && isset($_POST['refund_amount']) && isset($_POST['id_transaction']))
		{
			...
			$paypal_adpt_transaction_details = Db::getInstance()->getRow('SELECT * FROM '._DB_PREFIX_.'paypal_adpt_transaction WHERE id_order = '.(int)$_GET['id_order'].' AND type = \'payment\' AND id_shop = '.(int)$this->context->shop->id);
			if (isset($paypal_adpt_transaction_details['id_transaction']) && $paypal_adpt_transaction_details['id_transaction'] == Tools::getValue('id_transaction'))
			{
				/* Check how much has been refunded already on this order */
				$paypal_adpt_refunded = Db::getInstance()->getValue('SELECT SUM(amount) FROM '._DB_PREFIX_.'paypal_adpt_transaction WHERE id_order = '.(int)$_GET['id_order'].' AND type = \'refund\' AND id_shop = '.(int)$this->context->shop->id);
				if ($_POST['refund_amount'] <= number_format($paypal_adpt_transaction_details['amount'] - $paypal_adpt_refunded, 2, '.', ''))
					$this->_processRefund(Tools::getValue('id_transaction'), (float)Tools::getValue('refund_amount'), $paypal_adpt_transaction_details);
				else
				{
					$this->context->smarty->assign('paypal_adpt_refund', 0);
					$this->context->smarty->assign('paypal_adpt_refund_error', $this->l('You cannot refund more than').' '.Tools::displayPrice($paypal_adpt_transaction_details['amount'] - $paypal_adpt_refunded).' '.$this->l('on this order'));
				}
			}
			else
			{
				$this->context->smarty->assign('paypal_adpt_refund', 0);
				$this->context->smarty->assign('paypal_adpt_refund_error', $this->l('Invalid transaction ID, refund cannot be performed'));
			}
		}
	}
	...
	private function _processRefund($id_transaction, $amount, $original_transaction)
	{
		...
		if ($original_transaction['source'] == 'express' || ($original_transaction['source'] == 'standard' && Configuration::get('PAYPAL_USA_ACCOUNT') && Configuration::get('PAYPAL_USA_ACCOUNT') && Configuration::get('PAYPAL_USA_ACCOUNT')))
		{
			...
			if (Tools::strtoupper($result['ACK']) == 'SUCCESS' || Tools::strtoupper($result['ACK']) == 'SUCCESSWITHWARNING')
			{
				...
				$this->context->smarty->assign('paypal_adpt_refund', 1);
			}
			else
			{
				$this->context->smarty->assign('paypal_adpt_refund', 0);
				if (isset($result['L_SHORTMESSAGE0']))
					$this->context->smarty->assign('paypal_adpt_refund_error', $result['L_SHORTMESSAGE0'].' ('.$result['L_LONGMESSAGE0'].' - Err. code: '.$result['L_ERRORCODE0'].')');
			}
		}
		...
		elseif ($original_transaction['source'] == 'advanced')
		{
			...
			else
			{
				...
				if (isset($result['RESPMSG']))
					$this->context->smarty->assign('paypal_adpt_refund_error', $result['RESPMSG']);
			}
		}
	}
	...
	public function hookAdminOrder($params)
	{
		...
		if (Db::getInstance()->getValue('SELECT module FROM '._DB_PREFIX_.'orders WHERE id_order = '.(int)$_GET['id_order']) == $this->name)
		{
			...
			$paypal_adpt_transaction_details = Db::getInstance()->getRow('
			SELECT *
			FROM '._DB_PREFIX_.'paypal_adpt_transaction
			...
			$paypal_adpt_refund_details = Db::getInstance()->ExecuteS('
			...
			$paypal_adpt_transaction_details['source'] = $paypal_products[$paypal_adpt_transaction_details['source']];

			$this->context->smarty->assign(array(
				'paypal_adpt_more60d' => ((time() - strtotime($paypal_adpt_transaction_details['date_add'])) > (60 * 86400)), /* Do not allow refund if the order has been placed more than 60 days ago */
				'paypal_adpt_transaction_details' => $paypal_adpt_transaction_details,
				'paypal_adpt_refund_details' => $paypal_adpt_refund_details));
	...
	public function hookOrderConfirmation($params)
	{
		...
		{
			$this->smarty->assign('paypal_adpt_order', array('id' => $params['objOrder']->id, 'reference' => $params['objOrder']->reference, 'valid' => $params['objOrder']->valid));
		...
		{
			$this->smarty->assign('paypal_adpt_order', array('id' => $params['objOrder']->id,  'valid' => $params['objOrder']->valid));
	...
	public function hookShoppingCartExtra($params)
	{
		...
		{
			$this->smarty->assign('paypal_adpt_action', $this->context->link->getModuleLink('paypalusa', 'expresscheckout', array('pp_exp_initial' => 1)));
			$this->smarty->assign('paypal_adpt_merchant_country_is_mx', (Validate::isLoadedObject($this->_shop_country) && $this->_shop_country->iso_code == 'MX'));
	...
	public function addTransaction($type = 'payment', $details)
	{
		...
		return Db::getInstance()->Execute('
		INSERT INTO '._DB_PREFIX_.'paypal_adpt_transaction (type, source, id_shop, id_customer, id_cart, id_order,
